...
def set_path(experiment_dir_0, batchSize):
    ...
    return outckpts, trainpics, validationpics, outlogs, testPics, logPath
...
def parse_args():
    ...
    # loda dataset
    parser.add_argument("--dataset_name", type=str, default='pokemon',)
    parser.add_argument("--root_dataset_dir", type=str, default='dataset/pokemon-blip-captions/', )
    parser.add_argument("--clean_gen_data_dir", type=str, default='lora/clean/random/')
    parser.add_argument("--dataset_config_name", type=str, default=None)
    parser.add_argument("--secret", type=str, default='copyright')
    ...
    parser.add_argument("--select_train", type=str, default='train')
    parser.add_argument("--select_test", type=str, default='test')
    parser.add_argument("--train_data_dir", type=str, default='training_data/train/')
    parser.add_argument("--test_data_dir", type=str, default='training_data/test/')

    #output
    parser.add_argument("--root_model_dir", type=str, default='model/')
    parser.add_argument("--output_dir", type=str, default='test/')
    parser.add_argument("--save_train_data_dir", type=str, default="dataset/pokemon-blip-captions/training_data/train/")
    parser.add_argument("--cache_dir", type=str, default='cache/')
    parser.add_argument("--max_train_samples", type=int, default=None)

    ### PRE
    parser.add_argument("--output_dir_0", type=str,
                        default='model/pokemon/train/clean/30epoch/')
    parser.add_argument("--save_mid_lora_train_data_dir_0", type=str,
                        default="dataset/pokemon-blip-captions/lora/train/")
    parser.add_argument("--save_mid_lora_test_data_dir_0", type=str,
                        default="dataset/pokemon-blip-captions/lora/test/")
    parser.add_argument("--experiment_dir_0", type=str, default='data_save/')
...
def main():
    ...
    gen_local_csv(args.train_data_dir, args.train_data_dir + args.dataset_name + '.csv')
    gen_local_csv(args.test_data_dir, args.test_data_dir + args.dataset_name + '.csv')
    args.cache_dir = args.root_dataset_dir + args.cache_dir
    args.save_train_data_dir = args.root_dataset_dir + args.secret + '/mid_results/'

    args.output_dir = args.root_model_dir + args.dataset_name +'/' + args.secret + '/train/'
    ...
    outckpts, trainpics, validationpics, outlogs, testPics, logPath = set_path(args.experiment_dir_0, args.train_batch_size)
    down_num = 5
    lr = 0.0002
    beta1 = 0.5
    betamse = 10000
    betacleanA = betacleanB = beta = 1
    betaclean = 1
    betawatermark = 1
    betavgg = 1
    betapix = 1
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        if epoch == 1:
            bef_save_train_data_dir = args.train_data_dir
        ...
        if epoch == 0:
            ...
            print('bef_save_train_data_dir: ', bef_save_train_data_dir)
        ...
        for step, (batch, clean_batch, captions_) in enumerate(train_loader, 0):
            ...
            ssim_loss = 1 - ssim(generator_H_img, B_imgv, data_range=1.0)
            ...
            errR_mse = betamse * betawatermark * mse_loss(R_L_img, secret_imgv) + \
                       (1 - ssim(R_L_img, secret_imgv, data_range=1.0)) + criterion_pixelwise(R_L_img, secret_imgv)
            R_img_A = Rnet(A_imgv)
        ...
        for step_0, (batch_0, clean_batch_0, captions_0) in enumerate(train_loader_0, 0):
            ...
            gen_local_csv(cur_save_train_data_dir, cur_save_train_data_dir + args.dataset_name + '.csv')
...
def gen_local_csv(data_dir, save_dir):
    header = ['text', 'image']
    LIST = []
    IMG = glob(data_dir + '*.png')
    for ii in IMG:
        name = ii.split('/')[-1].split('-')[-1].split('.')[0]
        name = re.sub('_', ' ', name)
        tmp = (name, ii)
        LIST.append(tmp)

    with open(save_dir, 'w', encoding='utf-8') as file_obj:
